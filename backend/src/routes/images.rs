...
pub async fn get_image(Path(id): Path<String>, collections: &Collections) -> impl IntoResponse + use<> {
    if id.len() == 0 {
        return Json(json!({"status": StatusCodes::InvalidID}));
    }
    let image = collections.images.find_one(doc! { "_id":  ObjectId::parse_str(id).unwrap() }).await.unwrap_or(None);
    match image.clone() {
        Some(image) => Json(json!({"status": StatusCodes::Success, "image": image})),
